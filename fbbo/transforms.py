...
class Transform_Base(object):
    ...
    def __init__(self, Ytr: torch.Tensor):
        self.Ytr = Ytr

    # Transform the mean
    def scale_mean(self, mu: torch.Tensor) -> torch.Tensor:
        return mu

    # Reverse the transformation to the mean
    def unscale_mean(self, mu: torch.Tensor) -> torch.Tensor:
        return mu

    # Reverse the transformation to the variance
    def unscale_var(self, var: torch.Tensor) -> torch.Tensor:
        return var
...
class Transform_StandardizeRobustly(Transform_Base):
    ...
    def __init__(self, Ytr: torch.Tensor):
        ...
        self.Ytr_scale = torch.tensor(
            median_abs_deviation(Ytr_numpy), dtype=Ytr.dtype
        )
